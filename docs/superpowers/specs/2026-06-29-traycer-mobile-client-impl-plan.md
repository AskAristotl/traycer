...
| 3 — mobile surfaces (reuse-based) | ✅ done | **Inbox** built over the reused notifications stream (4 tests) + `/inbox` route + `useIsMobile` home branch; **Epic chat** + **read-only diff** delivered via the single-column canvas collapse reusing the real chat/diff tiles (3 + 106 tests green). Pixel-polish *inside* tiles still benefits from an on-device pass |
